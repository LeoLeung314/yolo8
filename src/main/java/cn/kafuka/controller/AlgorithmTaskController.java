...
public class AlgorithmTaskController {
    ...
    public ResultVo<Map<String, Object>> callShellScript(@RequestBody Map<String,Object> map) {
        ...
        /*// 改成 ↓↓↓  统一转成 Windows 反斜杠，去掉末尾空格
        if (workDir != null) {
            workDir = workDir.replace('/', File.separatorChar).trim();
        }

        if (workDir == null || !new File(workDir).isDirectory()) {
            throw new IllegalArgumentException("workDir 不是合法目录: " + workDir);
        }

        // 改成 ↓↓↓  整条用双引号包裹，内部空格就安全了
